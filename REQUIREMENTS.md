...
- 所有可见 conversations 统一纳入同一个 polling 集合
- 每 `5 分钟` 执行一轮全量增量 sweep
- 不再按 `DM / 活跃频道 / 非活跃频道` 分层调度
- 会话列表刷新: 每 `10 分钟`
...
- 软上限: `12 req/min`
- 硬上限: `20 req/min`
- 应主动平滑调用节奏，常态运行尽量保持在软上限附近，而不是依赖撞到硬上限后等待
- 任一 API 返回 `429` 时，严格遵守 `Retry-After`
- 单一 sweep 不得通过多条并发 polling loop 同时推进
